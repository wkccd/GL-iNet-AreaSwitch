...
# 获取当前区域函数
get_current_region() {
  region_hex=$(hexdump -C /dev/mtdblock11 | head -n 10 | grep -oE 'US|CN' | head -n 1)
...
while true; do
  ...
  case "$choice" in
    ...
    2)
      ...
      if [ "$confirm" = "yes" ]; then
        echo "正在切换为国区..."
        echo -n "CN" | dd of=/dev/mtdblock11 bs=1 seek=136 conv=notrunc
        sync
        echo "切换完成，正在重启路由器..."
        sleep 2
        reboot
